...
{ 
  ...
  // for RSA
  const unsigned char randomBytes[256]; //where the SHA output is stored
  unsigned char cipherText2[256];
  unsigned char decryptedText2[256];

  // Initializing the time collectors
  double aes128enc = 0;
  double aes128dec = 0;
  double sha256_1e2 = 0;
  double sha256_1e3 = 0;
...
double hashSHA256(int numBytes, unsigned char* rands)
{
  // This version can't do an arbitrary number of bytes at once. Has to be split up.
